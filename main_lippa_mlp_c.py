...
mlp_c = one_hidden_layer(1024)
mlp_c.load_state_dict(torch.load(model_pth_path)) # load the net
net = mlp_c
##################################################################
...
with open(csv_file_path, mode = 'w') as csv_file:
    ...
    root_advex_finder = lp_constructor_one_hidden_layer(net_dict)
    ...
    for sn in range(1000):
        ...
        tic = time.time()
        eps, x_adv = lippa(sample, root_advex_finder,lp_one_hidden_layer, net,pred_dict, eps_fgsm=0.5/256 ,tolerance=0.01, optim_time = 14, device = 'cpu',verbose = False, eps_h = EPS_H)
